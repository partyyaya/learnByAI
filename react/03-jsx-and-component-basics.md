...
如果不想只為了包裹而多一層無意義的 `<div>`（會弄髒 DOM 結構），可以用 **Fragment**——空標籤 `<>...</>`：
...
`<>...</>` 只負責「群組」，不會產生任何實際 DOM 節點。之後章節（例如第 5 章）會直接用到這個寫法。
...
JSX 裡不能直接寫 `if` 陳述式，要依條件顯示不同畫面時，常用三種寫法：

### 6.1 三元運算子（二選一）

```jsx
{isLoading ? <Spinner /> : <LessonList lessons={lessons} />}
```
...
`&&` 左邊若是**數字 `0`**，React 會把 `0` 印在畫面上——因為 `0` 會被短路運算式回傳，而 `0` 是「可以顯示的值」（不像 `false`、`null` 會被忽略）：

```jsx
{cartCount && <Badge count={cartCount} />}   // cartCount 為 0 時，畫面會冒出一個「0」
```
...
> 判斷準則：**用 `&&` 時，左邊永遠放布林（`xxx > 0`、`xxx != null`、`Boolean(xxx)`），不要直接放可能為 0 的數字。**
