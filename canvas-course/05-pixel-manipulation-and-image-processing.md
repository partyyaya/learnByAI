...
每個通道只有 0~255 共 256 階,而且是**整數**。每做一次運算,結果都得「四捨五入回整數 + 夾在 0~255 之間」,這一步會產生兩種不可逆的損失:

**① 捨入誤差會累積**——小數被砍掉,做越多層濾鏡誤差滾越大。以某像素亮度 `100`,先調暗到 1/3、再調亮 3 倍為例(理論上該回到 100):

```
100 ÷ 3 = 33.33…  →  存回 Uint8 只能是 33   ← 小數 .33 永遠消失
 33 × 3 = 99       →  存回 Uint8 是 99        ← 不是 100!掉了 1 階
```

**② 過曝/死黑不可逆(clipping)**——值頂到 0 或 255 邊界時,超出的資訊直接不見,反向運算也救不回:

```
過曝:200 × 2 = 400 → 夾成 255,再 ÷2 = 128   ← 想調回卻變 128,不是 200
死黑: 30 − 100 = −70 → 夾成 0,再 +100 = 100   ← 想調回卻變 100,不是 30
```

需要高精度(連續多層運算)時,把中間過程放進 `Float32Array`——**不四捨五入、不夾邊界**,直到最後一步才量化回 0~255:

```js
// ✅ 中間用 Float32Array,保住小數與超界值
const buf = Float32Array.from(imageData.data);   // 複製成浮點
for (let i = 0; i < buf.length; i++) {
  buf[i] = buf[i] / 3;   // 33.333…  ← 小數留著
  buf[i] = buf[i] * 3;   // 100      ← 完美還原(過曝的 400 也一樣留著,/2 還原成 200)
}
// 最後一步才「量化」回 8-bit:四捨五入 + 夾 0~255
for (let i = 0; i < buf.length; i++) {
  imageData.data[i] = Math.round(Math.max(0, Math.min(255, buf[i])));
}
```

> 下一節(5.9)的亮度滑桿用了另一招:**每次都從原圖 `original` 重算**、而不是在已處理結果上疊加——同樣避開誤差累積,是不想動用浮點緩衝時的簡易替代法。
