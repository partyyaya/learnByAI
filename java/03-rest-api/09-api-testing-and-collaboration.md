...
> 前面八章做了大量的設計決定：83 條端點、22 個 DTO、78 個錯誤碼、
...
| **能測到所有錯誤分支** | 78 個錯誤碼都要測 → 用真實資料庫製造 78 種情境是不現實的 |
...
| **P0** | 錯誤碼完整性（9.4.2 的 `@EnumSource`） | 一個測試涵蓋 78 個錯誤碼 |
...
83 個端點各寫 10 個手工測試 = 830 個測試 = 沒人維護。
...
| 可能原因 | 對應的缺失 |
|---|---|
| **錯誤格式測試不足**（5 個 vs 78 個錯誤碼） | 94% 的錯誤路徑沒測 |
...
□ P0：錯誤碼完整性（9.4.2 的 @EnumSource）—— 1 個測試涵蓋 78 個錯誤碼
     → 直接解決「錯誤格式測試只有 5 個」
...
| 01 | **83 條端點的 URL 表**、識別碼方案、非 CRUD 動作的五種手法 |
| 02 | **每條端點的方法／狀態碼契約**、15 條全域規則 |
| 03 | **22 個 DTO 全家族**、命名／金額／時間規範、列舉演進三層防護 |
| 04 | **67 個錯誤碼的錯誤目錄**、Problem Details 格式、前端消費程式碼 |
