...
`demo/` 底下會放一個純 Java（無框架）的**待辦事項 CLI**，一路從第 02 章長到第 12 章：
先用類別建模，再導入集合與 Stream，接著加例外處理、檔案持久化、併發匯入，最後補上完整測試。
...
**第 00～12 章全部完成**（含每章的驗收清單與附解答練習）。這一站已結業，下一站是 [02-spring-boot](../02-spring-boot/)。
