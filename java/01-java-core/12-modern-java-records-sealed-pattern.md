...
5. 依本章 12.16 節做一次 record 重構，看第 11 章的測試怎麼紅、怎麼引導你。

第 4 步和第 5 步特別值得——**它們是「讀過」和「做過」差距最大的兩章**。
...
02-spring-boot   ── IoC 容器、自動配置、設定管理
                    ⚠️ 你會發現 Spring 的每一個「魔法」，
                       都是這一站某個概念的自動化版本：
                       - DI 容器  = 第 03 章的手動組裝
                       - @Transactional = 第 03 章 3.13 節說的代理
