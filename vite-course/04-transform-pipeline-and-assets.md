...
### 延伸:`public/` 的檔換了,使用者卻看到舊的?

`public/` 的代價就在這裡:**檔名固定 = 沒有 hash = 瀏覽器會狠狠快取**。你換了 favicon,使用者重新整理卻還是舊的圖。`import` 靠 hash 自動破壞快取,放 `public/` 的檔得自己想辦法。以下兩種作法,檔案都還是放在 `public/`:

#### 作法一:加版本查詢字串(最簡單)

東西照樣放 `public/`,但**引用時掛個 `?v=`**:
...
更新圖時,把 `v=2` 改成 `v=3`。URL 一變,瀏覽器就當成新檔案重抓。**最低成本**,缺點是每次換圖都得手動改版本號。

> **為什麼有效**:瀏覽器是用「完整 URL」當快取的 key。`/favicon.ico` 和 `/favicon.ico?v=3` 對它來說是兩個不同的東西,所以會重新下載。

#### 作法二:用 HTTP 快取標頭控制(伺服器端)

檔名完全不動,改由**伺服器的回應標頭**決定快取行為。適合「無法改 HTML、只能動 server 設定」的情境:

```nginx
location = /favicon.ico {
    add_header Cache-Control "no-cache";   # 每次用之前,先問伺服器有沒有變
}
```

> **`no-cache` 不是「不要快取」**:它的意思是「**可以存,但每次用前都要先跟伺服器確認有沒有更新**」。瀏覽器會帶著 `ETag` 去問,沒變就回 `304 Not Modified`(省頻寬),變了才下載新檔。真正「完全不存」的是 `no-store`。

**怎麼選**:

| 你的情況 | 建議 |
|----------|------|
| 改得到 `index.html` | 作法一(`?v=`),最直覺 |
| 只能動 server 設定 / 想自動化 | 作法二(`Cache-Control` + `ETag`) |
| 偶爾才換圖 | 作法一就夠了 |

> **白話總結**:`public/` 的檔沒有 hash 護體,所以「換圖後快取沒更新」要自己處理——**改 HTML 就用 `?v=` 換版本號;只能動伺服器就用 `Cache-Control` 標頭。**
