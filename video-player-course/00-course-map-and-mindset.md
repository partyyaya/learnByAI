...
- 影音播放是「編碼 → 封裝 → 傳輸 → 解碼 → 渲染」的多層管線。每一層都可能成為卡頓、出錯或畫質損失的源頭，排查時要能定位是哪一層出問題：
  - **編碼（Encode）**：原始影像/聲音資料量極大（未壓縮的 1080p 可達每秒上百 MB），用 codec（如 H.264 / H.265 / AV1、音訊 AAC / Opus）壓縮成可傳輸的大小。這層決定了畫質與檔案大小的權衡。
  - **封裝（Container）**：把壓縮後的視訊、音訊、字幕軌道與 metadata 打包進容器格式（MP4 / fMP4 / TS / WebM）。容器決定了「怎麼切片、怎麼定位時間、能不能邊下邊播」。
  - **傳輸（Transport）**：透過 HTTP 漸進下載、或 HLS / DASH 串流（把影片切成小分段）、或 WebRTC 即時通道把資料送到播放端。這層也是自適應位元率（ABR）切換清晰度的所在。
  - **解碼（Decode）**：播放端把壓縮資料還原成一幀幀畫面與音訊取樣，通常交給硬體解碼器（GPU）以省電省效能；解碼跟不上就會掉幀。
  - **渲染（Render）**：把解碼後的影像畫到螢幕、聲音送到喇叭，並維持**音視訊同步（A/V sync）**。這層體感最直接——不同步或撕裂就是這裡出問題。
