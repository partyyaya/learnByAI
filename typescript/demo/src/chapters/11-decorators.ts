...
//
// 結構說明：每個小節（甚至小節內的每個範例）都各自包在 `{ ... }`
//       區塊裡，讓同名的 class/function（例如多次出現的 `User`、
//       `UserService`、`Calculator`、`UserController`）彼此區塊作用
//       域隔離，不會產生「重複宣告」錯誤——這與課程其他章節的慣例
//       一致。`import` 陳述式規定只能放在模組頂層，因此框架套件的
//       import 集中放在檔案最上方一次。
// ============================================================
...
// ===== 11.7 Accessor 裝飾器與 Decorator Metadata（TypeScript 5.2+）=====
{
  ...
  function trackChanges<This, Value>(
    ...
    // context.metadata 是一個掛在該類別 Symbol.metadata 上的共用物件，
    // 所有裝飾器都能往裡面寫東西（等於一個輕量版的 reflect-metadata）。
  ...
  // （用 (Symbol as any).metadata 存取，避免依賴 lib "esnext" 才有的型別宣告）
