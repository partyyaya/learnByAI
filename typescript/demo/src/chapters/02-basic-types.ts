// ============================================================
// 第 2 章：基本型別系統 — 範例整理
// 來源：typescript/02-basic-types.md
// 說明：每個獨立範例各自包在 { ... } 區塊內，利用區塊作用域避免同名
//       let/const/type/interface/enum/class 宣告互相衝突；用到前面
//       型別的範例會整段複製定義進來，讓每一塊自成一體、可獨立閱讀。
// ============================================================

// ===== 2.1 原始型別（Primitive Types）— string =====
{
...
// ===== 2.4 列舉 — 何時使用 Enum vs Union Type？ =====
// enum Role 與 type Role 同名衝突，故各自獨立成一個區塊。
{
  // Enum — 適合有邏輯分組的常數
  enum Role {
    Admin = "ADMIN",
...
// ===== 2.5 any =====
{
  ...
  anything = 42; // ✅ 不報錯
  anything = true; // ✅ 不報錯
  // anything.foo.bar; // ✅ 型別檢查不報錯，但執行時會丟 TypeError（故註解掉以便執行）

  // ⚠️ any 會讓 TypeScript 失去型別保護
}

// ===== 2.5 unknown — 安全的 any（TypeScript 3.0+） =====
{
...
console.log("第 2 章 基本型別系統 範例載入完成 ✅");
